...
else
{
    ...
    if(isset($_POST['submit_usr']))
    {
        ...
        $usr_id = $id['id'];
        $quer = "INSERT INTO  `webroute`.`rej_usr_exc` (`user` ,`site`, `action`)VALUES ('$usr_id', '$cat_add', 'deny')";
        mysql_query($quer);
    ...
    echo "&nbsp;Добавить пользовательское запрещение\n<br/>";
    ...
    echo "<br/>Удалить пользовательское запрещение\n<br/><table border=\"1\" cellspacing=\"0\" cellpadding=\"3\">";
    echo "<tr><td>пользователь</td><td>сайт</td></tr>";
    while($usr = mysql_fetch_assoc($usr1))
    {
        $usr_login = $usr['login'];
        $usr_id = $usr['id'];
        $select_cat_ex = "SELECT * FROM `webroute`.`rej_usr_exc` WHERE `user` = '$usr_id' and `action`='deny' ORDER BY `site`";
